...
describe('GET /api/catalog', () => {
	...
	it('returns catalog items with defaults', async () => {
		const items = [{ id: 1, name: 'Widget' }];
		vi.mocked(repositories.catalog.getCatalogItems).mockResolvedValue(items as any);

		const result = await GET({ url: makeUrl('/api/catalog') } as any);
	...
	it('passes search and category params', async () => {
		vi.mocked(repositories.catalog.getCatalogItems).mockResolvedValue([] as any);

		await GET({ url: makeUrl('/api/catalog', { search: 'test', category: 'parts', page: '2', limit: '10' }) } as any);
	...
	it('caps limit at 200', async () => {
		...
		await GET({ url: makeUrl('/api/catalog', { limit: '999' }) } as any);
	...
	it('throws 400 if search too long', async () => {
		...
		try {
			await GET({ url: makeUrl('/api/catalog', { search: longSearch }) } as any);
...
describe('GET /api/catalog/[id]', () => {
	...
	it('returns item by id', async () => {
		...
		vi.mocked(repositories.catalog.getCatalogItem).mockResolvedValue(item as any);
		const result = await GET_ID({ params: { id: '1' } } as any);
		expect((result as any).body).toEqual(item);
	});

	it('throws 404 if not found', async () => {
		vi.mocked(repositories.catalog.getCatalogItem).mockResolvedValue(undefined as any);
		try {
