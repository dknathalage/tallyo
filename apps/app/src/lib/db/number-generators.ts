...
export async function generateInvoiceNumber(): Promise<string> {
	...
	const result = await db
		.select({
			max_num: sql<number | null>`MAX(CAST(substring(${invoices.invoice_number} from 5) AS INTEGER))`
		})
		.from(invoices)
		.where(sql`${invoices.invoice_number} ~ '^INV-[0-9]+'`);
...
export async function generateEstimateNumber(): Promise<string> {
	...
	const result = await db
		.select({
			max_num: sql<number | null>`MAX(CAST(substring(${estimates.estimate_number} from 5) AS INTEGER))`
		})
		.from(estimates)
		.where(sql`${estimates.estimate_number} ~ '^EST-[0-9]+'`);
