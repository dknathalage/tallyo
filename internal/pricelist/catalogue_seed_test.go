...
func TestIngestClosesPriorVersionWindow(t *testing.T) {
	...
	if _, err := repo.Ingest(ctx, tid, "v1", "2025-07-01", "f1", item); err != nil {
		t.Fatalf("ingest v1: %v", err)
	}
	if _, err := repo.Ingest(ctx, tid, "v2", "2026-07-01", "f2", item); err != nil {
		t.Fatalf("ingest v2: %v", err)
	}

	// A 2025-26 service date resolves to v1; a 2026-27 date resolves to v2.
	if v, err := repo.ResolveVersionForDate(ctx, tid, "2025-08-01"); err != nil || v == nil || v.Label != "v1" {
		t.Fatalf("2025-08-01 should resolve to v1: got %v err=%v", v, err)
	}
	if v, err := repo.ResolveVersionForDate(ctx, tid, "2026-08-01"); err != nil || v == nil || v.Label != "v2" {
		t.Fatalf("2026-08-01 should resolve to v2: got %v err=%v", v, err)
	}
	// Boundary: the day before v2 still belongs to v1.
	if v, err := repo.ResolveVersionForDate(ctx, tid, "2026-06-30"); err != nil || v == nil || v.Label != "v1" {
