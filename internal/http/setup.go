...
var errNilUsers = errors.New("httpapi.NewSetupHandler: users repo is required")

// SetupHandler serves the first-run setup flow: owner creation while there are
// zero users, then a permanent 409 once an owner exists. It caches the
...
// the current user count. A nil repo is a programmer error.
func NewSetupHandler(users *auth.UsersRepo) (*SetupHandler, error) {
	if users == nil {
...
func (h *SetupHandler) CreateOwner(w http.ResponseWriter, r *http.Request) {
	...
	u, err := h.users.Create(ctx, req.Email, hash, "owner")
	if err != nil {
		WriteError(w, http.StatusInternalServerError, "internal error")
		return
	}
