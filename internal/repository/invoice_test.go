...
func TestInvoiceCreateNumbersAndTotals(t *testing.T) {
	...
	inv, err := repo.Create(ctx, tid, invoice.InvoiceInput{
	...
	inv2, err := repo.Create(ctx, tid, invoice.InvoiceInput{ParticipantID: pid, IssueDate: "2026-02-01", DueDate: "2026-02-28"},
...
func TestInvoiceGetWithBalance(t *testing.T) {
	...
	inv, err := repo.Create(ctx, tid, invoice.InvoiceInput{ParticipantID: pid, IssueDate: "2026-01-01", DueDate: "2026-01-31"},
		[]billing.LineItemInput{{Description: "X", Quantity: 1, UnitPrice: 100}})
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	if _, err := invoice.NewPayments(conn).Create(ctx, tid, invoice.PaymentInput{InvoiceID: inv.ID, Amount: 30, PaidAt: "2026-01-05"}); err != nil {
		t.Fatalf("pay: %v", err)
	}
...
func TestInvoiceUpdateAndStatus(t *testing.T) {
	...
	up, err := repo.Update(ctx, tid, inv.ID, invoice.InvoiceInput{ParticipantID: pid, IssueDate: "2026-01-01", DueDate: "2026-02-15", Tax: 5},
...
func TestInvoiceListAndDelete(t *testing.T) {
	...
	a, _ := repo.Create(ctx, tid, invoice.InvoiceInput{ParticipantID: pid, IssueDate: "2026-01-01", DueDate: "2026-01-31"},
		[]billing.LineItemInput{{Description: "X", Quantity: 1, UnitPrice: 1}})
...
func TestInvoiceParticipantStats(t *testing.T) {
	...
	inv, _ := repo.Create(ctx, tid, invoice.InvoiceInput{ParticipantID: pid, IssueDate: "2026-01-01", DueDate: "2026-01-31"},
		[]billing.LineItemInput{{Description: "X", Quantity: 1, UnitPrice: 200}})
	if _, err := invoice.NewPayments(conn).Create(ctx, tid, invoice.PaymentInput{InvoiceID: inv.ID, Amount: 50, PaidAt: "2026-01-05"}); err != nil {
		t.Fatalf("pay: %v", err)
	}
...
func TestInvoiceTenantIsolation(t *testing.T) {
	...
	inv, err := repo.Create(ctx, a, invoice.InvoiceInput{ParticipantID: pidA, IssueDate: "2026-01-01", DueDate: "2026-01-31"},
		[]billing.LineItemInput{{Description: "X", Quantity: 1, UnitPrice: 100}})
	if err != nil {
	...
	invB, err := repo.Create(ctx, b, invoice.InvoiceInput{ParticipantID: pidB, IssueDate: "2026-01-01", DueDate: "2026-01-31"},
		[]billing.LineItemInput{{Description: "X", Quantity: 1, UnitPrice: 1}})
	if err != nil {
