...
func TestInvoiceLineItemCustomItemRoundTrips(t *testing.T) {
	...
	if inv.LineItems[0].CatalogueItemID == nil || *inv.LineItems[0].CatalogueItemID != catalogueItemID {
		t.Fatalf("create customItemId: want %q got %v", catalogueItemID, inv.LineItems[0].CatalogueItemID)
	}
...
// TestInvoiceLineItemUnknownCatalogueItem422 verifies an unknown catalogue-item
// uuid on a line item is rejected by the line validator (422, with field detail).
func TestInvoiceLineItemUnknownCatalogueItem422(t *testing.T) {
	srv, uuid := newInvoiceServer(t)
	c := loggedInClient(t, srv.URL)
	clientID := createClient(t, c, srv.URL, uuid, "Acme")
