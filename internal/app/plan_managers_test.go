...
// newPlanManagerServer wires the plan-manager routes behind RequireSession+ResolveTenant the same
// way production does, plus a login route so tests can authenticate.
func newPlanManagerServer(t *testing.T) (*httptest.Server, string) {
	t.Helper()
...
func TestPlanManagerListEmptyReturnsArray(t *testing.T) {
	...
	resp := get(t, c, srv.URL+"/api/t/"+uuid+"/plan-managers")
	defer func() { _ = resp.Body.Close() }()
...
func TestPlanManagerGetBadID400(t *testing.T) {
	srv, uuid := newPlanManagerServer(t)
	c := loggedInClient(t, srv.URL)
	resp := get(t, c, srv.URL+"/api/t/"+uuid+"/plan-managers/abc")
	defer func() { _ = resp.Body.Close() }()
...
func TestPlanManagerCreateEmptyName400(t *testing.T) {
	srv, uuid := newPlanManagerServer(t)
	c := loggedInClient(t, srv.URL)
	resp := postJSON(t, c, srv.URL+"/api/t/"+uuid+"/plan-managers", `{"name":""}`)
	defer func() { _ = resp.Body.Close() }()
	if resp.StatusCode != http.StatusBadRequest {
...
func TestPlanManagerListSearchFilters(t *testing.T) {
	srv, uuid := newPlanManagerServer(t)
	c := loggedInClient(t, srv.URL)
	_ = createPlanManager(t, c, srv.URL, uuid, "Acme")
	_ = createPlanManager(t, c, srv.URL, uuid, "Globex")

	resp := get(t, c, srv.URL+"/api/t/"+uuid+"/plan-managers?search=acm")
	defer func() { _ = resp.Body.Close() }()
	if resp.StatusCode != http.StatusOK {
