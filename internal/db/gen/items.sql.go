// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.31.1
// source: items.sql

package gen

import (
	"context"
	"database/sql"
)

const countItems = `-- name: CountItems :one
SELECT COUNT(*) FROM items WHERE tenant_id = ?1 AND price_list_version_id = ?2
`

type CountItemsParams struct {
	TenantID  string `json:"tenant_id"`
	VersionID string `json:"version_id"`
}

func (q *Queries) CountItems(ctx context.Context, arg CountItemsParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, countItems, arg.TenantID, arg.VersionID)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const createItem = `-- name: CreateItem :one
INSERT INTO items (
    tenant_id, id, price_list_version_id, code, name, unit, category, unit_price, taxable, metadata
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
RETURNING id, price_list_version_id, code, name, unit, category, unit_price, taxable, metadata, tenant_id
`

type CreateItemParams struct {
	TenantID           string          `json:"tenant_id"`
	ID                 string          `json:"id"`
	PriceListVersionID string          `json:"price_list_version_id"`
	Code               string          `json:"code"`
	Name               string          `json:"name"`
	Unit               sql.NullString  `json:"unit"`
	Category           sql.NullString  `json:"category"`
	UnitPrice          sql.NullFloat64 `json:"unit_price"`
	Taxable            int64           `json:"taxable"`
	Metadata           sql.NullString  `json:"metadata"`
}

func (q *Queries) CreateItem(ctx context.Context, arg CreateItemParams) (Item, error) {
	row := q.db.QueryRowContext(ctx, createItem,
		arg.TenantID,
		arg.ID,
		arg.PriceListVersionID,
		arg.Code,
		arg.Name,
		arg.Unit,
		arg.Category,
		arg.UnitPrice,
		arg.Taxable,
		arg.Metadata,
	)
	var i Item
	err := row.Scan(
		&i.ID,
		&i.PriceListVersionID,
		&i.Code,
		&i.Name,
		&i.Unit,
		&i.Category,
		&i.UnitPrice,
		&i.Taxable,
		&i.Metadata,
		&i.TenantID,
	)
	return i, err
}

const deleteItemsForVersion = `-- name: DeleteItemsForVersion :exec
DELETE FROM items WHERE tenant_id = ?1 AND price_list_version_id = ?2
`

type DeleteItemsForVersionParams struct {
	TenantID  string `json:"tenant_id"`
	VersionID string `json:"version_id"`
}

func (q *Queries) DeleteItemsForVersion(ctx context.Context, arg DeleteItemsForVersionParams) error {
	_, err := q.db.ExecContext(ctx, deleteItemsForVersion, arg.TenantID, arg.VersionID)
	return err
}

const getItem = `-- name: GetItem :one
SELECT id, price_list_version_id, code, name, unit, category, unit_price, taxable, metadata, tenant_id FROM items WHERE tenant_id = ?1 AND id = ?2
`

type GetItemParams struct {
	TenantID string `json:"tenant_id"`
	ID       string `json:"id"`
}

func (q *Queries) GetItem(ctx context.Context, arg GetItemParams) (Item, error) {
	row := q.db.QueryRowContext(ctx, getItem, arg.TenantID, arg.ID)
	var i Item
	err := row.Scan(
		&i.ID,
		&i.PriceListVersionID,
		&i.Code,
		&i.Name,
		&i.Unit,
		&i.Category,
		&i.UnitPrice,
		&i.Taxable,
		&i.Metadata,
		&i.TenantID,
	)
	return i, err
}

const getItemByCode = `-- name: GetItemByCode :one
SELECT id, price_list_version_id, code, name, unit, category, unit_price, taxable, metadata, tenant_id FROM items
WHERE tenant_id = ?1 AND price_list_version_id = ?2 AND code = ?3
`

type GetItemByCodeParams struct {
	TenantID  string `json:"tenant_id"`
	VersionID string `json:"version_id"`
	Code      string `json:"code"`
}

func (q *Queries) GetItemByCode(ctx context.Context, arg GetItemByCodeParams) (Item, error) {
	row := q.db.QueryRowContext(ctx, getItemByCode, arg.TenantID, arg.VersionID, arg.Code)
	var i Item
	err := row.Scan(
		&i.ID,
		&i.PriceListVersionID,
		&i.Code,
		&i.Name,
		&i.Unit,
		&i.Category,
		&i.UnitPrice,
		&i.Taxable,
		&i.Metadata,
		&i.TenantID,
	)
	return i, err
}

const getItemIDByUUID = `-- name: GetItemIDByUUID :one
SELECT id FROM items WHERE tenant_id = ?1 AND id = ?2
`

type GetItemIDByUUIDParams struct {
	TenantID string `json:"tenant_id"`
	ID       string `json:"id"`
}

func (q *Queries) GetItemIDByUUID(ctx context.Context, arg GetItemIDByUUIDParams) (string, error) {
	row := q.db.QueryRowContext(ctx, getItemIDByUUID, arg.TenantID, arg.ID)
	var id string
	err := row.Scan(&id)
	return id, err
}

const listItems = `-- name: ListItems :many

SELECT id, price_list_version_id, code, name, unit, category, unit_price, taxable, metadata, tenant_id FROM items
WHERE tenant_id = ?1 AND price_list_version_id = ?2
ORDER BY code
`

type ListItemsParams struct {
	TenantID  string `json:"tenant_id"`
	VersionID string `json:"version_id"`
}

// Per-tenant price-list items (tenant-owned, scoped by tenant_id).
func (q *Queries) ListItems(ctx context.Context, arg ListItemsParams) ([]Item, error) {
	rows, err := q.db.QueryContext(ctx, listItems, arg.TenantID, arg.VersionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Item
	for rows.Next() {
		var i Item
		if err := rows.Scan(
			&i.ID,
			&i.PriceListVersionID,
			&i.Code,
			&i.Name,
			&i.Unit,
			&i.Category,
			&i.UnitPrice,
			&i.Taxable,
			&i.Metadata,
			&i.TenantID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const searchItems = `-- name: SearchItems :many
SELECT id, price_list_version_id, code, name, unit, category, unit_price, taxable, metadata, tenant_id FROM items
WHERE tenant_id = ?1 AND price_list_version_id = ?2
  AND ( (code     LIKE ?3 ESCAPE '\')
     OR (name     LIKE ?3 ESCAPE '\')
     OR (category LIKE ?3 ESCAPE '\')
     OR (unit     LIKE ?3 ESCAPE '\') )
ORDER BY code
LIMIT 50
`

type SearchItemsParams struct {
	TenantID  string `json:"tenant_id"`
	VersionID string `json:"version_id"`
	Q         string `json:"q"`
}

// All searchable fields (code, name, category, unit), tenant-scoped. sqlc.arg(q)
// is the escaped LIKE pattern; pair with ESCAPE '\'.
func (q *Queries) SearchItems(ctx context.Context, arg SearchItemsParams) ([]Item, error) {
	rows, err := q.db.QueryContext(ctx, searchItems, arg.TenantID, arg.VersionID, arg.Q)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Item
	for rows.Next() {
		var i Item
		if err := rows.Scan(
			&i.ID,
			&i.PriceListVersionID,
			&i.Code,
			&i.Name,
			&i.Unit,
			&i.Category,
			&i.UnitPrice,
			&i.Taxable,
			&i.Metadata,
			&i.TenantID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const upsertItem = `-- name: UpsertItem :one
INSERT INTO items (
    tenant_id, id, price_list_version_id, code, name, unit, category, unit_price, taxable, metadata
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
ON CONFLICT (price_list_version_id, code) DO UPDATE SET
    name = excluded.name,
    unit = excluded.unit,
    category = excluded.category,
    unit_price = excluded.unit_price,
    taxable = excluded.taxable,
    metadata = excluded.metadata
RETURNING id, price_list_version_id, code, name, unit, category, unit_price, taxable, metadata, tenant_id
`

type UpsertItemParams struct {
	TenantID           string          `json:"tenant_id"`
	ID                 string          `json:"id"`
	PriceListVersionID string          `json:"price_list_version_id"`
	Code               string          `json:"code"`
	Name               string          `json:"name"`
	Unit               sql.NullString  `json:"unit"`
	Category           sql.NullString  `json:"category"`
	UnitPrice          sql.NullFloat64 `json:"unit_price"`
	Taxable            int64           `json:"taxable"`
	Metadata           sql.NullString  `json:"metadata"`
}

func (q *Queries) UpsertItem(ctx context.Context, arg UpsertItemParams) (Item, error) {
	row := q.db.QueryRowContext(ctx, upsertItem,
		arg.TenantID,
		arg.ID,
		arg.PriceListVersionID,
		arg.Code,
		arg.Name,
		arg.Unit,
		arg.Category,
		arg.UnitPrice,
		arg.Taxable,
		arg.Metadata,
	)
	var i Item
	err := row.Scan(
		&i.ID,
		&i.PriceListVersionID,
		&i.Code,
		&i.Name,
		&i.Unit,
		&i.Category,
		&i.UnitPrice,
		&i.Taxable,
		&i.Metadata,
		&i.TenantID,
	)
	return i, err
}
