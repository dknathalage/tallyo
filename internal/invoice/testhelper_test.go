...
func seedSentPastDue(t *testing.T, conn *sql.DB, svc *Service, tenantID, participantID int64) *Invoice {
	t.Helper()
	ctx := tctx(tenantID)
...
// newInvoiceSvc creates a migrated DB, seeds a tenant+participant, and returns
// the invoice Service, Hub, tenantID, participantID.
func newInvoiceSvc(t *testing.T) (*Service, *realtime.Hub, int64, int64) {
	...
	hub := realtime.NewHub()
	return NewService(conn, hub, shift.NewShifts(conn)), hub, tenantID, participantID
}

// makeInvoice creates a single invoice for the tenant/participant and returns it.
func makeInvoice(t *testing.T, svc *Service, tenantID, participantID int64) *Invoice {
	t.Helper()
...
func seedInvoiceRepo(t *testing.T, conn *sql.DB, tenantID, participantID int64, unitPrice float64) int64 {
	t.Helper()
...
func seedInvoiceSvc(t *testing.T, invoices *InvoicesRepo, tenantID, participantID int64) *Invoice {
	t.Helper()
