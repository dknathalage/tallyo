...
<div class="space-y-2">
	{#if logo}
		<div class="flex items-start gap-3">
			<img src={logo} alt="Logo preview" class="h-16 w-16 rounded-lg border border-gray-200 object-contain" />
			<button
				type="button"
				onclick={removeLogo}
				class="cursor-pointer text-sm text-red-600 hover:text-red-700"
			>
				Remove
			</button>
		</div>
	{:else}
		<label class="flex cursor-pointer items-center gap-2 rounded-lg border border-dashed border-gray-300 px-4 py-3 text-sm text-gray-500 transition-colors hover:border-gray-400 hover:text-gray-600">
