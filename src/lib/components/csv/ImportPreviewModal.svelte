	...
	<div class="space-y-4">
		...
		<div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
			...
			<div class="rounded-lg bg-yellow-50 p-3 text-center">
				...
				<div class="text-xs text-yellow-600">Skipped Duplicates</div>
			</div>
		...
		{#if newClients && newClients.length > 0}
			<div class="rounded-lg border border-blue-200 bg-blue-50 p-3">
				<p class="text-sm font-medium text-blue-800">
					The following clients will be auto-created:
				</p>
		...
		{#if errors.length > 0}
			<div class="rounded-lg border border-red-200 bg-red-50 p-3">
				<p class="mb-2 text-sm font-medium text-red-800">
					Errors ({errors.length}{errors.length > 20 ? ', showing first 20' : ''}):
				</p>
