...
{:else}
	<div class="space-y-6">
		...
		<!-- Payer details / Edit form -->
		<div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
			{#if editing}
				<h2 class="mb-4 text-lg font-semibold text-gray-900 dark:text-white">{i18n.t('payer.editPayer')}</h2>
				<PayerForm initialData={payer} onsubmit={handleUpdate} />
			{:else}
				<dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
					<div>
						<dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{i18n.t('client.email')}</dt>
						<dd class="mt-1 text-sm text-gray-900 dark:text-white">{payer.email || '-'}</dd>
					</div>
					<div>
						<dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{i18n.t('client.phone')}</dt>
						<dd class="mt-1 text-sm text-gray-900 dark:text-white">{payer.phone || '-'}</dd>
					</div>
					<div class="sm:col-span-2">
						<dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{i18n.t('client.address')}</dt>
						<dd class="mt-1 whitespace-pre-line text-sm text-gray-900 dark:text-white">{payer.address || '-'}</dd>
					</div>
					{#if Object.keys(parseMetadataObj(payer.metadata)).length > 0}
						<div class="sm:col-span-2">
							<dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{i18n.t('common.additionalFields')}</dt>
							<dd class="mt-1">
								<div class="space-y-1">
		...
		<!-- Linked Clients -->
		<div>
			<h2 class="text-lg font-semibold text-gray-900 dark:text-white">{i18n.t('payer.linkedClients')}</h2>

			{#if linkedClients.length === 0}
				<div class="mt-4">
					<EmptyState title={i18n.t('payer.noLinkedClients')} message={i18n.t('payer.noLinkedClientsMessage')} />
				</div>
			{:else}
		...
		{#if history.length > 0}
			<div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
				<h2 class="mb-4 text-lg font-semibold text-gray-900 dark:text-white">{i18n.t('payer.changeHistory')}</h2>
				<div class="space-y-4">
	...
	<ConfirmDialog
		...
		title={i18n.t('payer.deleteConfirmTitle')}
		message={i18n.t('payer.deleteConfirmMessage', { name: payer.name })}
