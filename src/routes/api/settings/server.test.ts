...
describe('GET /api/settings', () => {
	...
	it('returns combined profile, taxRates, and columnMappings', () => {
		const profile = { business_name: 'Test Co' };
		const taxRates = [{ id: 1, rate: 10 }];
		const mappings = [{ id: 1, entity: 'catalog' }];
		vi.mocked(repositories.businessProfile.getBusinessProfile).mockReturnValue(profile as any);
		vi.mocked(repositories.taxRates.getTaxRates).mockReturnValue(taxRates as any);
