...
describe('GET /api/payments', () => {
	...
	it('returns empty array without invoiceId', () => {
		const result = GET({ url: makeUrl('/api/payments') } as any);
		expect((result as any).body).toEqual([]);
		expect(repositories.payments.getInvoicePayments).not.toHaveBeenCalled();
	});

	it('returns payments for invoiceId', () => {
		const payments = [{ id: 1, amount: 100 }];
		vi.mocked(repositories.payments.getInvoicePayments).mockReturnValue(payments as any);
		const result = GET({ url: makeUrl('/api/payments', { invoiceId: '5' }) } as any);
