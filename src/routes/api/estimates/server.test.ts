...
describe('GET /api/estimates', () => {
	...
	it('calls getEstimates with search, status, and pagination', () => {
		vi.mocked(repositories.estimates.getEstimates).mockReturnValue({ data: [], total: 0 } as any);
...
describe('GET /api/estimates/[id]', () => {
	...
	it('returns an estimate by id', () => {
		const mockEstimate = { id: 1, estimate_number: 'EST-001' };
		vi.mocked(repositories.estimates.getEstimate).mockReturnValue(mockEstimate as any);
	...
	it('throws 404 if estimate not found', () => {
		vi.mocked(repositories.estimates.getEstimate).mockReturnValue(undefined as any);
