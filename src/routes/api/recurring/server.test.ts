...
describe('GET /api/recurring', () => {
	...
	it('returns active templates by default', () => {
		...
		vi.mocked(repositories.recurringTemplates.getRecurringTemplates).mockReturnValue(templates as any);
		const result = GET({ url: makeUrl('/api/recurring') } as any);
	...
	it('returns all templates when all=true', () => {
		vi.mocked(repositories.recurringTemplates.getRecurringTemplates).mockReturnValue([] as any);
		GET({ url: makeUrl('/api/recurring', { all: 'true' }) } as any);
...
describe('GET /api/recurring/[id]', () => {
	...
	it('returns template by id', () => {
		...
		vi.mocked(repositories.recurringTemplates.getRecurringTemplate).mockReturnValue(template as any);
		const result = GET_ID({ params: { id: '1' } } as any);
		expect((result as any).body).toEqual(template);
	});

	it('throws 404 if not found', () => {
		vi.mocked(repositories.recurringTemplates.getRecurringTemplate).mockReturnValue(undefined as any);
		try {
