...
import type { CreateCatalogItemInput } from '$lib/repositories/interfaces/types.js';
...
export const POST: RequestHandler = async ({ request }) => {
	const body = (await request.json()) as { action?: string; ids?: number[] } & Record<string, unknown>;
	...
	try {
		const id = await repositories.catalog.createCatalogItem(data as unknown as CreateCatalogItemInput);
