...
describe('GET /api/payers', () => {
	...
	it('returns payers without search', () => {
		const payers = [{ id: 1, name: 'Acme' }];
		vi.mocked(repositories.payers.getPayers).mockReturnValue(payers as any);
		const result = GET({ url: makeUrl('/api/payers') } as any);
	...
	it('passes search param', () => {
		vi.mocked(repositories.payers.getPayers).mockReturnValue([] as any);
		GET({ url: makeUrl('/api/payers', { search: 'acme' }) } as any);
	...
	it('throws 400 if search too long', () => {
		...
		try {
			GET({ url: makeUrl('/api/payers', { search: longSearch }) } as any);
...
describe('GET /api/payers/[id]', () => {
	...
	it('returns payer by id', () => {
		const payer = { id: 1, name: 'Acme' };
		vi.mocked(repositories.payers.getPayer).mockReturnValue(payer as any);
		const result = GET_ID({ params: { id: '1' } } as any);
		expect((result as any).body).toEqual(payer);
	});

	it('throws 404 if not found', () => {
		vi.mocked(repositories.payers.getPayer).mockReturnValue(undefined as any);
		try {
