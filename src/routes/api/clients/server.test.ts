...
describe('GET /api/clients', () => {
	...
	it('calls getClients with search and pagination', () => {
		const mockData = { data: [], total: 0 };
		vi.mocked(repositories.clients.getClients).mockReturnValue(mockData as any);

		const url = makeUrl('/api/clients', { search: 'acme', page: '2', limit: '25' });
		const result = GET({ url } as any);
	...
	it('calls getClients with defaults when no params', () => {
		vi.mocked(repositories.clients.getClients).mockReturnValue({ data: [], total: 0 } as any);
...
describe('GET /api/clients/[id]', () => {
	...
	it('returns a client by id', () => {
		...
		vi.mocked(repositories.clients.getClient).mockReturnValue(mockClient as any);
	...
	it('throws 404 if client not found', () => {
		vi.mocked(repositories.clients.getClient).mockReturnValue(undefined as any);

		try {
			GET_ID({ params: { id: '1' } } as any);
			expect.unreachable('should have thrown');
		} catch (e: any) {
	...
	it('throws 400 for negative id', () => {
		try {
			GET_ID({ params: { id: '-5' } } as any);
			expect.unreachable('should have thrown');
		} catch (e: any) {
			expect(e.status).toBe(400);
		}
