...
export const POST: RequestHandler = async ({ request }) => {
	...
	repositories.aiChat.addMessage({ session_id, role: 'user', content: message });

	// Build full conversation history
	const history = repositories.aiChat.getSessionMessages(session_id);
	...
	const stream = new ReadableStream({
		async start(ctrl) {
			...
			const assistantMsgId = repositories.aiChat.addMessage({
				session_id,
				role: 'assistant',
				content: '',
				is_streaming: 1
			...
			try {
				...
				repositories.aiChat.finalizeMessage(
					assistantMsgId,
					fullContent,
				...
				const sess = repositories.aiChat.getSession(session_id);
				if (sess?.title === 'New Chat') {
					repositories.aiChat.updateSessionTitle(
						session_id,
				...
				repositories.aiChat.finalizeMessage(assistantMsgId, `Error: ${msg}`);
			}
			ctrl.close();
		}
	});
