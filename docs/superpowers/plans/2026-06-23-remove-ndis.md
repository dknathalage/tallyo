...
- [ ] **Step 1:** Read `validation.go`. `ValidateFilling` has **two callers**: `internal/invoice/service.go:164` and `internal/session/service.go:468` (`priceItem`, reached from session reprice/add-item/divide at `service.go:181,330,384,411`). Since `Validate` and `ValidateFilling` now both fill from `unit_price`, **keep `ValidateFilling` as a thin alias** (`return v.validate(ctx, …)`) so neither caller changes — simplest, no ripple. (Do NOT drop it and repoint only invoice — that breaks the session package.)
...
- [ ] **Step 3:** Fix `internal/pricelist/*` (compiler worklist): drop `ItemPrice`, `ResolveZonePrice`, prices repo methods, the prices handler+route, and strip `Zone`/`PriceCap`/`Quotable` from `Match` + the `zone` param from `SearchForDate`. **Import dead-code (compiler-invisible, ~`repository.go:278,285,349-365`):** removing the `UpsertItemPrice` call leaves an empty `for zone, capPtr := range it.Prices` loop, a `priceCount` stuck at 0, the unused `ImportItem.Prices map[string]*float64` field, and a meaningless `IngestResult.PriceCount`. Delete the loop, the `Prices` field, and `priceCount`; drop `PriceCount` from `IngestResult` (and the frontend reading it, if any) since generic import has no per-zone prices.
...
- [ ] **Step 4:** Update/delete the stray `internal/importer/mapping.go:26` `ponytail:` comment ("multi-zone NDIS cap import is a later extension") so the Task 3.2 grep ends clean.
- [ ] **Step 5:** Commit: `docs: remove NDIS from CLAUDE.md + ERDs`
...
- [ ] **Step 3:** End-to-end smoke on a fresh data dir (real server): `signup` (confirm NO zone field in payload, generic tenant) → create a client (name only) → `POST …/price-list/import/commit` a CSV → invoice a catalogue item by code (prices from `unit_price`) + a free-form line → expect 201, correct subtotal/total. **Also exercise the session pricing path** (the 2nd `ValidateFilling` caller): record a session → add a coded item → confirm it prices from `unit_price`, no zone error. Confirm server log has no errors.
